...
import org.openpnp.machine.photon2.exceptions.FeedFailureException;
import org.openpnp.machine.photon2.exceptions.FeederHasNoLocationOffsetException;
import org.openpnp.machine.photon2.exceptions.NoSlotAddressException;
import org.openpnp.machine.photon2.exceptions.UnconfiguredSlotException;
import org.openpnp.machine.photon2.protocol.ErrorTypes;
import org.openpnp.machine.photon2.protocol.PhotonBus;
import org.openpnp.machine.photon2.protocol.PhotonBusInterface;
import org.openpnp.machine.photon2.protocol.commands.*;
import org.openpnp.machine.photon2.sheets.FeederPropertySheet;
import org.openpnp.machine.photon2.sheets.GlobalConfigPropertySheet;
...
public class PhotonFeederTwo extends ReferenceFeeder {
    ...
    public void initializeIfNeeded() throws Exception {
        ...
        if (response == null) {
            ...
            PhotonFeederTwo otherFeeder = findByHardwareId(response.uuid);

            // If we don't know about that feeder, let's go ahead and create it
            if (otherFeeder == null) {
                otherFeeder = new PhotonFeederTwo();
    ...
    /**
     * The PhotonFeederTwo assumes you have a physical slot that is numbered 1 - 254. That
    ...
    public void setSlotAddress(Integer slotAddress) {
        ...
        if (slotAddress != null) {
            ...
            PhotonFeederTwo otherFeeder = findBySlotAddress(slotAddress);
            if (otherFeeder != null) {
    ...
    public static PhotonFeederTwo findByHardwareId(String hardwareId) {
        for (Feeder feeder : Configuration.get().getMachine().getFeeders()) {
            if (!(feeder instanceof PhotonFeederTwo)) {
                continue;
            }

            PhotonFeederTwo photonFeederTwo = (PhotonFeederTwo) feeder;

            // Are we explicitly asking for feeders with null hardware ID?
            if (photonFeederTwo.hardwareId == null && hardwareId == null) {
                return photonFeederTwo;
            }

            if (photonFeederTwo.hardwareId != null && photonFeederTwo.hardwareId.equals(hardwareId)) {
                return photonFeederTwo;
            }
        }
    ...
    public static PhotonFeederTwo findBySlotAddress(int slotAddress) {
        for (Feeder feeder : Configuration.get().getMachine().getFeeders()) {
            if (!(feeder instanceof PhotonFeederTwo)) {
                continue;
            }
            ...
            if (photonFeederTwo.slotAddress != null && photonFeederTwo.slotAddress.equals(slotAddress)) {
                return photonFeederTwo;
            }
        }
    ...
    public static void findAllFeeders(FeederSearchProgressConsumer progressUpdate) throws Exception {
        ...
        List<PhotonFeederTwo> feedersToAdd = new ArrayList<>();
        ...
        for (int address = 1; address <= maxFeederAddress; address++) {
            ...
            if (response == null) {
                PhotonFeederTwo otherFeeder = findBySlotAddress(address);
                if (otherFeeder != null) {
                    otherFeeder.slotAddress = null;
                    otherFeeder.initialized = false;
                }
        ...
        for (PhotonFeederTwo feeder : feedersToAdd) {
            Configuration.get().getMachine().addFeeder(feeder);
        }
    }

    private static CvPipeline createDefaultPipeline(PipelineType type) {
        // TODO: configure xml for nice default pipeline
        // try {
        //     String xml = IOUtils.toString(BlindsFeeder.class
        //             .getResource("ReferencePushPullFeeder-"+type.tag+"Pipeline.xml")); // TODO: Create custom default config
        ...
        try {
            String xml = IOUtils.toString(ReferenceBottomVision.class
                    .getResource("ReferenceBottomVision-"+"Default"+"Pipeline.xml"));
            return new CvPipeline(xml);
        }
        catch (Exception e) {
            throw new Error(e);
